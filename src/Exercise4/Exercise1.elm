module Exercise4.Exercise1 exposing (..)
...
getMu : Float
getMu =
    mu getAttributValues

getStd : Float
getStd =
    std getAttributValues
...
main =
    let
        ...
        f_values = 
            getAttributValues |> List.indexedMap (\i _ -> (toFloat (i+1) - 0.5) / (List.length getAttributValues |> toFloat))

        xyDataQuantils =
           dataToXyDataQuantil f_values getAttributValues

        quantiles : List Float
        quantiles =
            f_values |> List.map (\f -> Maybe.withDefault 0 (Statistics.quantile f getAttributValues))
