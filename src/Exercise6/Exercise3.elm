module Exercise6.Exercise3 exposing (..)
...
import TypedSvg.Attributes exposing (fill, class,color, fontFamily, fontSize, textAnchor, transform, viewBox, points, d)
import TypedSvg.Attributes.InPx exposing (cx, cy, height, r, width, x, y, x1, x2, y1, y2, strokeWidth)
import TypedSvg.Core exposing (Svg)
import TypedSvg.Types exposing (px, AnchorAlignment(..), Length(..), Transform(..), Paint(..), Length(..))
...
filterAndReduceCars : CarType -> List CarField -> List Car -> MultiDimData
filterAndReduceCars carType carFields carList =
    let
        filtered =
            List.filterMap
                (\car -> carToPoint carType carFields car)
                carList
    in
    MultiDimData
        (List.map .description carFields)
        filtered


carToPoint : CarType -> List CarField -> Car -> Maybe MultiDimPoint
carToPoint carType carFields car =
    if car.carType == carType then
        let
            fields =
                List.map (\f -> f.accessor car) carFields
    ...
    case List.head x of
        Just head ->
            case List.tail x of
                Just tail ->
                    Maybe.map2
                        (\justHead justTail -> justHead :: justTail)
                        head
                        (maybeList tail)
...
parallelCoordinatesPlot width ar model =
    let
        height =
            width / ar

        dataPerDimension : List (List Float)
        dataPerDimension =
            List.indexedMap
                (\dim _ ->
                    List.filterMap
                        (\point ->
                            Array.get dim (Array.fromList point.value)
                        )
                        model.data
                )
        ...
        yScales =
            List.map
                (\dimensionData ->
                    Scale.linear (height - 2 * padding, 0) (wideExtent dimensionData)
                )
                dataPerDimension
        ...
        yAxes =
            List.map
                (\scale -> Axis.left [Axis.tickCount tickCount] scale) yScales

        yData : List (ContinuousScale Float, Svg msg, String)
        yData =
            List.Extra.zip3 yScales yAxes model.dimDescription

        numDims =
            List.length yData

        xScale =
            Scale.linear (0, width - 2 * padding) (0, toFloat numDims - 1)

        xAxis =
            Axis.bottom [ Axis.tickCount numDims, Axis.tickFormat (\_ -> "")] xScale
        ...
        lines =
            List.map 
                (\point ->
                    let
                        lineGenerator : Int -> (Float, ContinuousScale Float) ->  Maybe ( Float, Float )
                        lineGenerator dim (value, scale) =
                            Just 
                                (Scale.convert xScale (toFloat dim)
                                , Scale.convert scale value
                                )
                            
                        linePath data =
                            List.indexedMap lineGenerator data
                                |> Shape.line Shape.linearCurve
                    in
                    linePath (List.Extra.zip point.value yScales)
                )
                model.data
    in
    ...
    svg 
        ...
        [ g
            ...
            [ xAxis
            , text_
                [ x (half (Scale.rangeExtent xScale))
                , y (padding / 2)
                , TypedSvg.Attributes.textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text "Dimensions" ]
            ]
        , g
            [ transform [ Translate padding padding ] ]
            (List.indexedMap
                (\dim ( scale, axis, description ) ->
                    g
                        [ transform
                            [ Translate
                                (Scale.convert xScale (toFloat dim))
                        ...
                        , text_
                            [ x 0
                            , y (Tuple.second (Scale.rangeExtent scale) - (padding / 2))
                            , TypedSvg.Attributes.textAnchor AnchorMiddle
                            ]
                            [ TypedSvg.Core.text description ]
                        ]
                )
                yData
            )
...
init =
    Model
        ...
        (Reorderable.fromList
            [ CarField "cityMPG" (\car -> Maybe.map toFloat car.cityMPG)
            , CarField "retailPrice" (\car -> Maybe.map toFloat car.retailPrice)
            , CarField "dealerCost" (\car -> Maybe.map toFloat car.dealerCost)
            , CarField "carLen" (\car -> Maybe.map toFloat car.carLen)
            ]
        )

viewCarTypeButton : Model -> CarType -> Html Msg
viewCarTypeButton model carType =
    let
        selectedLabel =
            if model.carType == carType then
                "✓ "
    ...
    in
    Html.button
        [ Html.Events.onClick (SelectCarType carType) ]
        [ Html.text (selectedLabel ++ carTypeLabel carType) ]
...
view model =
    let
        ...
        filteredCarsCarType =
            filterAndReduceCars model.carType
                (Reorderable.toList model.carFields)
                cars

        numberFilteredCarsCarType =
            length filteredCarsCarType.data
    in
    Html.div []
        [ Html.p []
            [ Html.text "Select car type: "
            , viewCarTypeButton model Small_Sporty_Compact_Large_Sedan
            , viewCarTypeButton model Sports_Car
            , viewCarTypeButton model SUV
            , viewCarTypeButton model Wagon
            , viewCarTypeButton model Minivan
            , viewCarTypeButton model Pickup
            ]
        , Html.p []
            [ Html.text "Sort car fields: "
            , viewCarFieldsList model
            ]
        , Html.p []
            [ Html.text
                ("Cars: "
                    ...
                    ++ ", filtered cars: "
                    ++ String.fromInt numberFilteredCarsCarType
                )
            ]
        , parallelCoordinatesPlot w (16 / 9) filteredCarsCarType
        ]

viewCarFieldsList : Model -> Html Msg
viewCarFieldsList model =
    let
        fields =
            Reorderable.toList model.carFields
            ...
            (\index field ->
                let
                    upButton =
                        if index > 0 then
                            Just
                                (Html.button
                                    [ Html.Events.onClick (MoveCarField Up index) ]
                                    [ Html.text "↑" ]
                                )
                    ...
                    downButton =
                        if index < (List.length fields) - 1 then
                            Just
                                (Html.button
                                    [ Html.Events.onClick (MoveCarField Down index) ]
                                    [ Html.text "↓" ]
                                )
                ...
                Html.li
                    ...
                    (List.filterMap identity
                        [ Just (Html.text (field.description ++ " "))
                        , upButton
                        , downButton
                        ]
                    )
...
update msg model =
    case msg of
        ...
        MoveCarField direction index ->
            case direction of
                Up ->
                    { model | carFields = Reorderable.moveUp index model.carFields }

                Down ->
                    { model | carFields = Reorderable.moveDown index model.carFields }



type Msg
    = SelectCarType CarType
    | MoveCarField MoveDirection Int
...
type alias Model =
    { carType : CarType
    , carFields : Reorderable.Reorderable CarField
    }

type alias CarField =
    { description : String
    , accessor : Car -> Maybe Float
    }
