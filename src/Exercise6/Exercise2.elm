module Exercise6.Exercise2 exposing (..)
...
import TypedSvg.Attributes exposing (fill, class,color, fontFamily, fontSize, textAnchor, transform, viewBox, points, d, strokeOpacity)
import TypedSvg.Attributes.InPx exposing (cx, cy, height, r, width, x, y, x1, x2, y1, y2, strokeWidth)
import TypedSvg.Core exposing (Svg)
import TypedSvg.Types exposing (px, AnchorAlignment(..), Length(..), Transform(..), Paint(..), Length(..), Opacity(..))
...
filterAndReduceCars : CarType -> List Car -> MultiDimData
filterAndReduceCars carType carList =
    let 
        filtered = List.filterMap
            (\car -> carToPoint carType car)
            carList
    in
    MultiDimData ["cityMPG", "retailPrice", "dealerCost", "carLength"] filtered

carToPoint : CarType -> Car -> Maybe MultiDimPoint
carToPoint carType car =
...
init : Model
init =
    Model Small_Sporty_Compact_Large_Sedan
...
view model =
    let
        ...
        filteredCarsCarType =
            filterAndReduceCars model.carType cars
